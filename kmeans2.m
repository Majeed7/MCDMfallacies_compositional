...
if hadNaNs
    ...
    function cellout = loopBody(rep,S)
           ...
           case 'numeric'
            ...
            case {'plus','kmeans++'}
                ...
                for ii = 2:k                    
                    ...
                    sampleProbability = full(minDist/denominator);
        ...
        try % catch empty cluster errors and move on to next rep
            ...
            totsumD = full(sum(sumD(nonempties)));
        ...
        function converged = batchUpdate()
            ...
            while true
                ...
                totsumD = full(sum(D((idx-1)*n + (1:n)')));
        ...
        function converged = onlineUpdate()
            ...
            while iter < maxit
                ...
                oidx = idx(moved);
                nidx = nidx(moved);
                totsumD = totsumD + full(Del(moved,nidx) - Del(moved,oidx));
